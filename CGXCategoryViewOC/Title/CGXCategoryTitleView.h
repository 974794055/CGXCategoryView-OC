...
@class CGXCategoryTitleView;

@protocol CGXCategoryTitleViewDataSource <NSObject>

@optional
/**
 如果将CGXCategoryTitleView嵌套进UITableView的cell，每次重用的时候，CGXCategoryTitleView进行reloadData时，会重新计算所有的title宽度。
 所以该应用场景，需要UITableView的cellModel缓存titles的文字宽度，再通过该代理方法返回给CGXCategoryTitleView。
...
@interface CGXCategoryTitleView : CGXCategoryIndicatorView

@property (nonatomic, weak) id<CGXCategoryTitleViewDataSource> titleDataSource;
...
@property (nonatomic, assign) CGXCategoryTitleLabelAnchorPointStyle titleLabelAnchorPointStyle;  //titleLabel锚点位置，用于调整titleLabel缩放时的基准位置。默认为：CGXCategoryTitleLabelAnchorPointStyleCenter
