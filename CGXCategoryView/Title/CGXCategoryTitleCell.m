...
- (void)reloadData:(CGXCategoryBaseCellModel *)cellModel {
    ...
    CGXCategoryTitleCellModel *myCellModel = (CGXCategoryTitleCellModel *)cellModel;
    ...
    if (myCellModel.titleLabelStrokeWidthEnabled) {
        if (myCellModel.selectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            CGXCategoryCellSelectedAnimationBlock block = [self preferredTitleStrokeWidthAnimationBlock:myCellModel attributedString:attributedString];
            [self addSelectedAnimationBlock:block];
        }else {
    ...
    if (myCellModel.titleLabelMaskEnabled) {
        ...
        self.titleLabel.layer.mask = nil;
        if (myCellModel.selectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            CGXCategoryCellSelectedAnimationBlock block = [self preferredTitleColorAnimationBlock:myCellModel];
            [self addSelectedAnimationBlock:block];
        }else {
    ...
    if (myCellModel.cellBackgroundColorGradientEnabled) {
        if (myCellModel.isSelected) {
            self.contentView.backgroundColor = myCellModel.cellBackgroundSelectedColor;
        }else {
...
- (CGXCategoryCellSelectedAnimationBlock)preferredTitleZoomAnimationBlock:(CGXCategoryTitleCellModel *)cellModel baseScale:(CGFloat)baseScale {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            //将要选中，scale从小到大插值渐变
            cellModel.titleLabelCurrentZoomScale = [CGXCategoryFactory interpolationFrom:cellModel.titleLabelNormalZoomScale to:cellModel.titleLabelSelectedZoomScale percent:percent];
        }else {
            //将要取消选中，scale从大到小插值渐变
            cellModel.titleLabelCurrentZoomScale = [CGXCategoryFactory interpolationFrom:cellModel.titleLabelSelectedZoomScale to:cellModel.titleLabelNormalZoomScale percent:percent];
        }
...
- (CGXCategoryCellSelectedAnimationBlock)preferredTitleStrokeWidthAnimationBlock:(CGXCategoryTitleCellModel *)cellModel attributedString:(NSMutableAttributedString *)attributedString {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleLabelCurrentStrokeWidth = [CGXCategoryFactory interpolationFrom:cellModel.titleLabelNormalStrokeWidth to:cellModel.titleLabelSelectedStrokeWidth percent:percent];
        }else {
            //将要取消选中，StrokeWidth从大到小插值渐变
            cellModel.titleLabelCurrentStrokeWidth = [CGXCategoryFactory interpolationFrom:cellModel.titleLabelSelectedStrokeWidth to:cellModel.titleLabelNormalStrokeWidth percent:percent];
        }
...
- (CGXCategoryCellSelectedAnimationBlock)preferredTitleColorAnimationBlock:(CGXCategoryTitleCellModel *)cellModel {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleCurrentColor = [CGXCategoryFactory interpolationColorFrom:cellModel.titleNormalColor to:cellModel.titleSelectedColor percent:percent];
        }else {
            //将要取消选中，textColor从titleSelectedColor到titleNormalColor插值渐变
            cellModel.titleCurrentColor = [CGXCategoryFactory interpolationColorFrom:cellModel.titleSelectedColor to:cellModel.titleNormalColor percent:percent];
        }
